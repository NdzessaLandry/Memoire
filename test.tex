...
à l'international.
	...
	de qualité retenus pour évaluer la solution livrée. Ce document
	a été élaboré en cohérence avec les résultats de l'analyse
	statistique des données de l'enquête du \gls{minepat} présentée
	au chapitre précédent, dont les enseignements ont directement
	informé la définition des besoins fonctionnels de la plateforme.
	...
	connaissances de la plateforme et le moteur de recommandation
	...
	\begin{itemize}
		\item la création et la gestion de profils d'entreprises~;
		\item la saisie et la mise à jour des besoins exprimés par
		les entreprises~;
		\item la cartographie géographique et sectorielle des entreprises
		enregistrées~;
		\item la mise en relation automatisée des entreprises sur la base
		d'un algorithme de recommandation fondé sur la similarité
		cosinus calculée dans l'espace factoriel de l'ACM~;
		\item la gestion des interactions entre entreprises mises en
		relation~;
		\item l'administration et le suivi de la plateforme par les
		agents du \gls{minepat}.
	\end{itemize}
	
	Sont explicitement exclus du périmètre de la version actuelle~:
	la gestion des transactions commerciales entre entreprises, le
	traitement des paiements en ligne, la signature électronique des
	contrats de partenariat, et l'intégration avec les systèmes
	d'information tiers des administrations partenaires (DGI, CNPS,
	OHADA). Ces fonctionnalités pourront être envisagées dans une
	version ultérieure de la plateforme.
	...
	\subsection{L'entreprise (acteur principal)}
	...
	est représentée sur la plateforme par un ou plusieurs utilisateurs
	physiques disposant d'un compte personnel sécurisé.
	...
	entreprises, de la modération des contenus publiés, du suivi
	...
	\begin{tcolorbox}[
		colback=lightgray, colframe=primary,
		title={\textbf{UC-01 — Inscription d'une entreprise}},
		fonttitle=\bfseries\small\color{white},
		colbacktitle=primary, arc=4pt, boxrule=0.8pt
		]
		...
		\begin{enumerate}
			...
			\item Un courriel de confirmation est envoyé à l'adresse
			électronique renseignée~;
			\item L'administrateur valide l'inscription après vérification
			des informations~;
			\item L'entreprise reçoit une notification de validation et
			peut accéder à son espace personnel.
		\end{enumerate}
		\textbf{Exceptions~:} Adresse électronique déjà utilisée~;
		informations incomplètes ou incohérentes~; délai de validation
		dépassé.
	\end{tcolorbox}
	...
	\begin{tcolorbox}[
		colback=lightgray, colframe=primary,
		title={\textbf{UC-02 — Authentification}},
		fonttitle=\bfseries\small\color{white},
		colbacktitle=primary, arc=4pt, boxrule=0.8pt
		]
		...
		\textbf{Scénario nominal~:}
		\begin{enumerate}
			\item L'utilisateur saisit son adresse électronique et son
		...
		avant verrouillage temporaire du compte)~; compte non encore validé
		par l'administrateur.
	\end{tcolorbox}
	
	\subsection{Gestion du profil de l'entreprise}
	
	\begin{tcolorbox}[
		colback=lightgray, colframe=primary,
		title={\textbf{UC-03 — Compléter et mettre à jour le profil}},
		fonttitle=\bfseries\small\color{white},
		colbacktitle=primary, arc=4pt, boxrule=0.8pt
		]
		...
		\textbf{Scénario nominal~:}
		\begin{enumerate}
			\item L'utilisateur accède à la section \og{}Mon profil\fg{}
			depuis son tableau de bord~;
			\item Il complète ou met à jour les informations relatives à son
			entreprise~: localisation précise, branche d'activité,
			biens et services proposés, taille, ancienneté, coordonnées
			et site web~;
			\item Il enregistre les modifications~;
			\item Le système met à jour les données et régénère les
			recommandations de partenaires en conséquence.
		\end{enumerate}
		\textbf{Exceptions~:} Données incohérentes~; format de fichier
		non supporté pour les pièces jointes éventuelles.
	\end{tcolorbox}
	
	\subsection{Gestion des besoins}
	
	\begin{tcolorbox}[
		colback=lightgray, colframe=primary,
		title={\textbf{UC-04 — Saisir et gérer ses besoins}},
		fonttitle=\bfseries\small\color{white},
		colbacktitle=primary, arc=4pt, boxrule=0.8pt
		]
		\textbf{Acteur principal~:} Entreprise \\
		...
		\textbf{Scénario nominal~:}
		\begin{enumerate}
			\item L'utilisateur accède à la section \og{}Mes besoins\fg{}~;
			...
			\item Il renseigne une description détaillée du besoin~;
			\item Il indique le degré de priorité (urgent, important,
			secondaire)~;
		...
		\end{enumerate}
		\textbf{Exceptions~:} Description insuffisamment détaillée~;
		catégorie de besoin non sélectionnée.
	\end{tcolorbox}
	
	\subsection{Moteur de recommandation et mise en relation}
	
	\begin{tcolorbox}[
		colback=lightgray, colframe=primary,
		title={\textbf{UC-05 — Consulter les recommandations de partenaires}},
		fonttitle=\bfseries\small\color{white},
		colbacktitle=primary, arc=4pt, boxrule=0.8pt
		]
		\textbf{Acteur principal~:} Entreprise \\
		...
		\begin{enumerate}
			...
			\item Le système calcule la similarité cosinus entre le profil
			factoriel de l'entreprise et ceux des autres entreprises
			enregistrées~;
			\item Les cinq entreprises présentant les scores de similarité
			les plus élevés sont affichées, accompagnées de leur profil
			synthétique et du score de compatibilité calculé~;
			\item L'utilisateur peut affiner les recommandations en appliquant
			des filtres (région, secteur, taille)~;
			\item Il peut accéder au profil complet d'une entreprise
			recommandée et initier un contact.
		\end{enumerate}
		\textbf{Exceptions~:} Aucune entreprise compatible trouvée~;
		profil insuffisamment renseigné pour générer des recommandations
		pertinentes.
	\end{tcolorbox}
	
	\begin{tcolorbox}[
		colback=lightgray, colframe=primary,
		title={\textbf{UC-06 — Contacter un partenaire potentiel}},
		fonttitle=\bfseries\small\color{white},
		colbacktitle=primary, arc=4pt, boxrule=0.8pt
		]
		\textbf{Acteur principal~:} Entreprise \\
		\textbf{Préconditions~:} L'entreprise est authentifiée et a
		consulté les recommandations \\
		\textbf{Scénario nominal~:}
		\begin{enumerate}
			\item L'utilisateur sélectionne une entreprise recommandée~;
			\item Il accède à son profil public~;
			\item Il rédige un message de prise de contact via le formulaire
			intégré à la plateforme~;
			\item Le message est transmis à l'entreprise destinataire par
			notification interne et par courriel~;
			\item L'entreprise destinataire peut accepter, refuser ou
			différer la mise en relation.
		\end{enumerate}
		\textbf{Exceptions~:} Message vide~; entreprise destinataire
		ayant désactivé les contacts entrants.
	\end{tcolorbox}
	
	\subsection{Administration de la plateforme}
	
\begin{tcolorbox}[
	colback=gray!10, % Plus clair pour une meilleure lisibilité du texte noir
	colframe=primary,
	title={UC-07 --- Tableau de bord administrateur}, % --- pour un tiret cadratin propre
	fonttitle=\bfseries\small\color{white},
	colbacktitle=primary, arc=4pt, boxrule=0.8pt
	]
	...
	\begin{enumerate}
		...
		saisis, nombre de mises en relation initiées, répartition
		géographique et sectorielle des entreprises ;
		\item Il gère les demandes d'inscription en attente de validation ;
		\item Il accède à la liste complète des entreprises et peut
		consulter, modifier ou suspendre tout profil ;
		\item Il peut exporter les données agrégées sous format CSV
...
\end{tcolorbox}
	...
	La plateforme doit être en mesure de répondre aux requêtes des
	utilisateurs dans un délai maximal de \textbf{3 secondes} pour
	les pages de consultation de profils et d'affichage des
	recommandations, et de \textbf{5 secondes} pour les opérations
	impliquant le calcul de la similarité cosinus sur l'ensemble
	de la base d'entreprises. Ces seuils ont été définis en tenant
	compte des contraintes de connectivité caractéristiques du
	contexte camerounais, où les débits internet disponibles peuvent
	être sensiblement inférieurs à ceux des pays à infrastructure
	numérique plus développée. La plateforme doit par ailleurs être
	dimensionnée pour supporter simultanément au moins
	\textbf{500 utilisateurs connectés} sans dégradation significative
	des performances, ce seuil étant défini sur la base des effectifs
	d'entreprises potentiellement utilisatrices identifiés lors de
	l'enquête du \gls{minepat}.
	...
	\subsection{Exigences d'accessibilité et d'ergonomie}
	...
	\subsection{Exigences de maintenabilité et d'évolutivité}
	...
	La plateforme doit garantir un taux de disponibilité
	(\textit{uptime}) d'au moins \textbf{99\,\%} sur une base mensuelle,
	soit un temps d'indisponibilité maximal de 7 heures 12 minutes par
	mois. Les opérations de maintenance planifiées doivent être réalisées
	en dehors des heures de forte utilisation, idéalement entre 23h00
	et 05h00 (heure de Yaoundé), et annoncées aux utilisateurs avec
	un préavis minimal de 48 heures.
	
	%=============================================================
	\section{Modélisation des données : Modèle Conceptuel des Données}
	\label{sec:mcd}
	%=============================================================
	
	Le Modèle Conceptuel des Données (MCD), élaboré selon la méthode
	MERISE, représente la structure logique des informations gérées par
	la plateforme \textit{Business Partnership}, indépendamment de toute
	considération technique relative au système de gestion de bases de
	données retenu. Il identifie les entités du système, leurs attributs
	et les associations qui les relient, en précisant les cardinalités
	de chaque association \citep{tardieu_1983}.
	
	\subsection{Entités et attributs}
	
	Les entités principales du modèle sont les suivantes~:
	
	\begin{itemize}
		
		\item \textbf{Entreprise}~: entité centrale du modèle. Attributs~:
		\texttt{id} (identifiant unique), \texttt{raison\_sociale},
		\texttt{statut\_juridique}, \texttt{taille}, \texttt{date\_creation},
		\texttt{email}, \texttt{telephone}, \texttt{site\_web},
		\texttt{localisation}, \texttt{date\_inscription},
		\texttt{statut\_validation}~;
		
		\item \textbf{Region}~: entité référentielle. Attributs~:
		\texttt{id}, \texttt{nom\_region}~;
		
		\item \textbf{SecteurActivite}~: entité référentielle. Attributs~:
		\texttt{id}, \texttt{nom\_secteur}, \texttt{type\_secteur}
		(primaire/secondaire/tertiaire)~;
		
		\item \textbf{BrancheActivite}~: entité référentielle. Attributs~:
		\texttt{id}, \texttt{nom\_branche}~;
		
		\item \textbf{Besoin}~: entité décrivant les besoins exprimés.
		Attributs~: \texttt{id}, \texttt{type\_besoin},
		\texttt{detail\_besoin}, \texttt{priorite},
		\texttt{date\_saisie}, \texttt{statut}~;
		
		\item \textbf{MiseEnRelation}~: entité associative matérialisant
		une mise en relation entre deux entreprises. Attributs~:
		\texttt{id}, \texttt{date\_initiation}, \texttt{statut}
		(en attente/acceptée/refusée), \texttt{score\_similarite}~;
		
		\item \textbf{Utilisateur}~: entité représentant les comptes
		d'accès à la plateforme. Attributs~: \texttt{id},
		\texttt{email}, \texttt{mot\_de\_passe\_hache},
		\texttt{role} (entreprise/administrateur),
		\texttt{date\_creation\_compte}, \texttt{dernier\_acces}.
	...
	\begin{itemize}
		...
		\item \textbf{Entreprise -- Region}~: une entreprise est
		implantée dans une et une seule région~; une région peut
		accueillir plusieurs entreprises. Cardinalité~: (1,1) -- (0,n)~;
		
		\item \textbf{Entreprise -- SecteurActivite}~: une entreprise
		appartient à un et un seul secteur d'activité~; un secteur peut
		regrouper plusieurs entreprises. Cardinalité~: (1,1) -- (0,n)~;
		
		\item \textbf{Entreprise -- BrancheActivite}~: une entreprise
		opère dans une branche d'activité~; une branche peut regrouper
		plusieurs entreprises. Cardinalité~: (1,1) -- (0,n)~;
		
		\item \textbf{Entreprise -- Besoin}~: une entreprise peut
		exprimer plusieurs besoins~; chaque besoin est associé à une
		et une seule entreprise. Cardinalité~: (1,n) -- (1,1)~;
		
		\item \textbf{Entreprise -- MiseEnRelation}~: une entreprise
		peut initier plusieurs mises en relation et en recevoir
		plusieurs~; chaque mise en relation implique exactement deux
		entreprises. Cardinalité~: (0,n) -- (2,2)~;
		
		\item \textbf{Entreprise -- Utilisateur}~: un compte utilisateur
		est rattaché à une et une seule entreprise~; une entreprise peut
		disposer de plusieurs comptes utilisateurs. Cardinalité~:
		(1,n) -- (1,1).
		
	\end{itemize}
	
	%=============================================================
	\section{Diagramme des cas d'usage}
	\label{sec:use_case}
	%=============================================================
